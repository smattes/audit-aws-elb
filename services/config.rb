...
function setTextRollup() {
    ...
    textRollup += 'Total Number of Cloud Objects: ' + numberOfViolations + "\\n";
    textRollup += 'Rollup' + "\\n";
...
COMPOSITE::coreo_uni_util_jsrunner.elb-tags-rollup.return
  ...
  payload_type 'text'
  endpoint ({
      :to => '${AUDIT_AWS_ELB_ALERT_RECIPIENT}', :subject => 'CloudCoreo rollup report for elb rule results on PLAN::stack_name :: PLAN::name'
  })
end
